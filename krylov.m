...
elseif method == "lstrs"
    epsilon.Delta = tol;
    if ~isempty(H)
        [s, lambda] = lstrs(H, g, delta, epsilon);
    else
        [s, lambda] = lstrs(@func_handle, g, delta, epsilon);
    end     
end
...
% the initialization
epsilon.Delta = tol;  % for lstrs
epsilon.HC = tol;  % for lstrs
% epsilon.Int = tol;  % for lstrs
% epsilon.alpha = tol;  % for lstrs
% epsilon.nu = tol;  % for lstrs
if ~isempty(H)
    n = size(H, 1);
else
    n = size(L, 2);
...
%               [idx_list(1:k), idx_list(1:k-1), idx_list(2:k)], ...
%               [t_diag(1:k), t_offdiag(1:k-1), t_offdiag(1:k-1)] ...
%               ), gprime, delta, epsilon, @eig_gateway);
        [hk, lambda] = trid_newton(sparse( ...
              [idx_list(1:k), idx_list(2:k), idx_list(1:k-1)], ...
              [idx_list(1:k), idx_list(1:k-1), idx_list(2:k)], ...
              [t_diag(1:k), t_offdiag(1:k-1), t_offdiag(1:k-1)] ...
              ), gamma0, lambda, delta, tol);
      else
        [hk, lambda] = lstrs(sparse( ...
              [idx_list(1:k), idx_list(2:k), idx_list(1:k-1)], ...
              [idx_list(1:k), idx_list(1:k-1), idx_list(2:k)], ...
