%
% eigs_lstrs.m - to be used with MATLAB 7
%

function  varargout = eigs_lstrs(varargin)
%EIGS_LSTRS  Find a few eigenvalues and eigenvectors of a matrix using ARPACK
%   D = EIGS_LSTRS(A) returns a vector of A's 6 largest magnitude eigenvalues.
%   A must be square and should be large and sparse.
%
%   [V,D] = EIGS_LSTRS(A) returns a diagonal matrix D of A's 6 largest magnitude
%   eigenvalues and a matrix V whose columns are the corresponding
%   eigenvectors.
%
%   [V,D,FLAG] = EIGS_LSTRS(A) also returns a convergence flag. If FLAG is 0 then
%   all the eigenvalues converged; otherwise not all converged.
%
%   EIGS_LSTRS(A,B) solves the generalized eigenvalue problem A*V == B*V*D. B
%   must be symmetric (or Hermitian) positive definite and the same size as
%   A. EIGS_LSTRS(A,[],...) indicates the standard eigenvalue problem A*V == V*D.
%
%   EIGS_LSTRS(A,K) and EIGS_LSTRS(A,B,K) return the K largest magnitude eigenvalues.
%
%   EIGS_LSTRS(A,K,SIGMA) and EIGS_LSTRS(A,B,K,SIGMA) return K eigenvalues. If SIGMA is:
...
%   If SIGMA is a real or complex scalar including 0, EIGS_LSTRS finds the
...
%
%   EIGS_LSTRS(A,K,SIGMA,OPTS) and EIGS_LSTRS(A,B,K,SIGMA,OPTS) specify options:
...
%
%   EIGS_LSTRS(AFUN,N) accepts the function AFUN instead of the matrix A. AFUN is
...
%   by OPTS.isreal and OPTS.issym. In all these EIGS_LSTRS syntaxes, EIGS_LSTRS(A,...)
%   may be replaced by EIGS_LSTRS(AFUN,N,...).
%
%   Example:
%      A = delsq(numgrid('C',15));  d1 = eigs_lstrs(A,5,'SM');
%
...
%      d2 = eigs_lstrs(@(x)dnRk(x,'C',15),n,5,'SM',opts);
%
...
%   EIGS_LSTRS provides the reverse communication interface to ARPACK library
%   routines. EIGS_LSTRS attempts to provide an interface for as many different
...
%   $Revision: 1.40 $  $Date: 2001/04/15 12:00:10 $
%
%   MODIFIED BY:  Marielba Rojas
%                 mr@imm.dtu.dk
%
%   DATE:         14 March 2007
%
%   Modification: when called with 4 to 6 output parameters 'eigs_lstrs' returns:
%                 - the number of converged Ritz values in the 4th argument
%                 - the smallest of the Ritz values computed in the 5th argument
%                 - the first column of the Lanczos matrix (V) in the 6th argument
%
%
...
% Now have enough information to do early return on cases EIGS_LSTRS does not
...
if strcmp(eigs_sigma,'SM') || ~ischar(eigs_sigma)
   % eigs_lstrs(A,B,k,scalarSigma) or eigs_lstrs(A,B,k,'SM'), B may be []
   ...
   mode = 3;
elseif isempty(B)
   % eigs_lstrs(A,k,stringSigma) or eigs_lstrs(A,[],k,stringSigma), stringSigma~='SM'
   % A*x = lambda*x
   % => OP = A and B = I
   mode = 1;
else
   % eigs_lstrs(A,B,k,stringSigma), stringSigma~='SM'
   % A*x = lambda*B*x
...
if strcmp(classAB,'single')
   error('MATLAB:eigs_lstrs:single', ...
      'EIGS_LSTRS does not support single precision inputs.')
end

% The ARPACK routines return to EIGS_LSTRS many times per each iteration but we
...
%          v, ldv, iparam, ipntr, workd, workl, lworkl, info );
   ...
   if (info < 0)
      error('MATLAB:eigs_lstrs:ARPACKroutineError', ...
   ...
   % The ARPACK reverse communication parameter ido tells EIGS_LSTRS what to do
   switch ido
      ...
      case 3 % ido==3 => EIGS_LSTRS does not know how to compute shifts
         % setting iparam(1) = ishift = 1 ensures this never happens
         warning('MATLAB:eigs_lstrs:WorklShiftsUnsupported', ...
            ['EIGS_LSTRS does not support computing the shifts in workl.' ...
            ' Returning immediately.'])
         ido = intconvert(99);
      case 99 % ido==99 => ARPACK is done
      otherwise
         error('MATLAB:eigs_lstrs:UnknownReverseCommParamFromARPACK',...
...
% checkInputs error checks the inputs to EIGS_LSTRS and also derives some
%   variables from them:
...
% afunNargs is the range of EIGS_LSTRS' varargin that are to be passed as
%   trailing parameters to the function as in afun(X,P1,P2,...).
   function [A,Amatrix,isrealprob,issymA,n,B,classAB,k, ...
      ...
      if Amatrix
         ...
         [m,n] = size(A);
         if (m ~= n)
            error('MATLAB:eigs_lstrs:NonSquareMatrixOrFunction',...
         ...
         if ~isscalar(n) || ~isreal(n)
            error('MATLAB:eigs_lstrs:NonPosIntSize', nstr)
         end
         if issparse(n)
            n = full(n);
         end
         if (round(n) ~= n)
            warning('MATLAB:eigs_lstrs:NonPosIntSize',['%s\n         ' ...
      ...
      % Is B present in the eigs_lstrs call or not?
      Bpresent = true;
      ...
      if (nargin < (3-Amatrix))
         ...
         if ~isempty(B) % allow eigs_lstrs(A,[],k,sigma,opts);
            if isscalar(B) && (n ~= 1)
               ...
               % eigs_lstrs(4,2,...) assumes A=4, B=2, NOT A=4, k=2
               if ~isfloat(B) || ~isequal(size(B),[n,n])
                  error('MATLAB:eigs_lstrs:BsizeMismatchAorNotSPDorNotChol', Bstr)
               end
      ...
      % argOffset tells us where to get the eigs_lstrs inputs K, SIGMA and OPTS.
      % If A is really the function afun, then it also helps us find the
      % trailing parameters in eigs_lstrs(afun,n,[B],k,sigma,opts,P1,P2,...)
      % Values of argOffset:
      %  0: Amatrix is false and Bpresent is true:
      %     eigs_lstrs(afun,n,B,k,sigma,opts,P1,P2,...)
      %  1: Amatrix and Bpresent are both true, or both false
      %     eigs_lstrs(A,B,k,sigma,opts)
      %     eigs_lstrs(afun,n,k,sigma,opts,P1,P2,...)
      %  2: Amatrix is true and Bpresent is false:
      %     eigs_lstrs(A,k,sigma,opts)
      argOffset = Amatrix + ~Bpresent;

      if Amatrix && ((nargin - Bpresent)>4)
         error('MATLAB:eigs_lstrs:TooManyInputs', 'Too many inputs.')
      end
      ...
      if (nargin < (4-argOffset))
         ...
         if ~isnumeric(k) || ~isscalar(k) || ~isreal(k) || (k>n)
            error('MATLAB:eigs_lstrs:NonIntegerEigQty', kstr)
         end
         if issparse(k)
            k = full(k);
         end
         if (round(k) ~= k)
            warning('MATLAB:eigs_lstrs:NonIntegerEigQty',['%s\n         ' ...
      ...
      % passed in to EIGS_LSTRS. eigs_sigma may be either a scalar, including 0,
      ...
      if (nargin < (5-argOffset))
         % default: eigs_lstrs 'LM' => ARPACK which='LM', sigma=0
         eigs_sigma = 'LM';
         whch = 'LM';
         sigma = 0;
      else
         ...
         if ischar(eigs_sigma)
            % eigs_lstrs(string) => ARPACK which=string, sigma=0
            if ~isequal(size(eigs_sigma),[1,2])
               error('MATLAB:eigs_lstrs:EigenvalueRangeNotValid', ...
            ...
            if strcmp(eigs_sigma,'SM')
               % eigs_lstrs('SM') => ARPACK which='LM', sigma=0
               whch = 'LM';
            else
               % eigs_lstrs(string), where string~='SM' => ARPACK which=string, sigma=0
               whch = eigs_sigma;
            end
            sigma = zeros(classAB);
         else
            % eigs_lstrs(scalar) => ARPACK which='LM', sigma=scalar
            if ~isfloat(eigs_sigma) || ~isscalar(eigs_sigma)
               error('MATLAB:eigs_lstrs:EigenvalueShiftNonScalar',...
      ...
      % randomly generated within eigs_lstrs.
      info = intconvert(1);
      if isrealprob
      ...
      if (nargin >= (6-argOffset))
         ...
         if ~isempty(B) && (isfield(opts,'cholB') || isfield(opts,'permB'))
            if isfield(opts,'cholB')
               ...
               if (cholB ~= false) && (cholB ~= true)
                  error('MATLAB:eigs_lstrs:InvalidOptsCholB', ...
               ...
               if isfield(opts,'permB')
                  if issparse(B) && cholB
                     ...
                     if ~isvector(permB) || ~isequal(sort(permB(:)),(1:n)')
                        error('MATLAB:eigs_lstrs:InvalidOptsPermB',...
                           'opts.permB must be a permutation of 1:n.')
                     end
                  else
                     warning('MATLAB:eigs_lstrs:IgnoredOptionPermB', ...
         ...
         if isfield(opts,'tol')
            if ~isfloat(tol) || ~isscalar(opts.tol) || ~isreal(opts.tol) || (opts.tol<=0)
               error('MATLAB:eigs_lstrs:InvalidOptsTol',...
         ...
         if isfield(opts,'p')
            ...
            if ~isnumeric(p) || ~isscalar(p) || ~isreal(p) || (p<=0) || (p>n)
               error('MATLAB:eigs_lstrs:InvalidOptsP', pstr)
            end
            if issparse(p)
               p = full(p);
            end
            if (round(p) ~= p)
               warning('MATLAB:eigs_lstrs:NonIntegerVecQty',['%s\n         ' ...
         ...
         if isfield(opts,'maxit')
            ...
            if ~isnumeric(maxit) || ~isscalar(maxit) || ~isreal(maxit) || (maxit<=0)
               error('MATLAB:eigs_lstrs:OptsMaxitNotPosInt', str)
            end
            if issparse(maxit)
               maxit = full(maxit);
            end
            if (round(maxit) ~= maxit)
               warning('MATLAB:eigs_lstrs:NonIntegerIterationQty',['%s\n         ' ...
         ...
         if isfield(opts,'v0')
            if ~isfloat(opts.v0) || ~isequal(size(opts.v0),[n,1])
               error('MATLAB:eigs_lstrs:WrongSizeOptsV0',...
                  'Start vector opts.v0 must be n-by-1.')
            end
            if isrealprob
               if ~isreal(opts.v0)
                  error('MATLAB:eigs_lstrs:NotRealOptsV0',...
         ...
         if isfield(opts,'disp')
            ...
            if ~isnumeric(eigs_display) || ~isscalar(eigs_display) || ...
               ...
               error('MATLAB:eigs_lstrs:NonIntegerDiagnosticLevel', dispstr)
            end
            if (round(eigs_display) ~= eigs_display)
               warning('MATLAB:eigs_lstrs:NonIntegerDiagnosticLevel', ...
                  '%s\n         Rounding diagnostic level.',dispstr)
               eigs_display = round(eigs_display);
            end
         end
         if isfield(opts,'cheb')
            warning('MATLAB:eigs_lstrs:ObsoleteOptionCheb', ...
               ['Ignoring polynomial acceleration opts.cheb' ...
               ' (no longer an option).']);
         end
         if isfield(opts,'stagtol')
            warning('MATLAB:eigs_lstrs:ObsoleteOptionStagtol', ...
      ...
      if ~Amatrix
         ...
         % in eigs_lstrs(afun,n,[B],k,sigma,opts,P1,P2,...). If there are no
         % trailing parameters in eigs_lstrs, then afunNargs is a 1-by-0 empty
      ...
      if isrealprob && issymA
         if (k > n-1)
            if (n >= 6)
               warning('MATLAB:eigs_lstrs:TooManyRequestedEigsForRealSym', ...
                  '%s',knstr)
            end
            useeig = true;
         end
         ...
         if (k > n-2)
            if (n >= 7)
               warning('MATLAB:eigs_lstrs:TooManyRequestedEigsForComplexNonsym', ...
                  '%s',knstr)
            end
            useeig = true;
         end
      ...
      if isrealprob && issymA
         if ~isreal(sigma)
            error('MATLAB:eigs_lstrs:ComplexShiftForRealSymProblem',...
               ['For real symmetric problems, eigenvalue shift sigma must' ...
               ' be real.'])
         end
      else
         if ~isrealprob && issymA && ~isreal(sigma)
            warning('MATLAB:eigs_lstrs:ComplexShiftForHermitianProblem', ...
      ...
      if isrealprob && issymA
         if strcmp(whch,'LR')
            whch = 'LA';
            warning('MATLAB:eigs_lstrs:SigmaChangedToLA', ...
               ['For real symmetric problems, sigma value ''LR''' ...
               ' (Largest Real) is now ''LA'' (Largest Algebraic).'])
         end
         if strcmp(whch,'SR')
            whch = 'SA';
            warning('MATLAB:eigs_lstrs:SigmaChangedToSA', ...
         ...
         if strcmp(whch,'BE')
            warning('MATLAB:eigs_lstrs:SigmaChangedToLM', ...
      ...
      if isempty(p)
         ...
         if isrealprob && issymA
            if (p <= k)
               error('MATLAB:eigs_lstrs:InvalidOptsPforRealSymProb',...
                  ['For real symmetric problems, must have number of' ...
                  ' basis vectors opts.p > k.'])
            end
         else
            if (p <= k+1)
               error('MATLAB:eigs_lstrs:InvalidOptsPforComplexOrNonSymProb',...
...
%-------------------------------------------------------------------------%
   function [RB,RBT,perm] = CHOLfactorB
      ...
      if cholB
         ...
         % CHOL(B) was not passed into EIGS_LSTRS
         if (mode == 1) && ~isempty(B)
...
%-------------------------------------------------------------------------%
   function printTimings
      % Print the time taken for each major stage of the EIGS_LSTRS algorithm
      if (mode == 1)
...
if (nargout >= 4)
   varargout{4} = double(iparam(5));                % number of converged Ritz values
   if (nargout >= 5)
      ind = double(ipntr(6));                   % where Ritz values start in 'workl'
      ncv = double(ncv);                        % number of Ritz values
      varargout{5} = min(workl(ind:ind+ncv-1)); % minimum Ritz value
      if (nargout >= 6)
         varargout{6} = v(:,1);                 % first column of Lanczos basis matrix 'v'
      end
  end
end
